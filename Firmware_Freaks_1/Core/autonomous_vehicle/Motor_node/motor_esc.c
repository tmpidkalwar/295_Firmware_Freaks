...
static void motor_esc__set_steer_angle(float servo_pwm_val) {
  ...
  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, servo_pwm_val*100);  //Set Servo Duty cycle
}
...
static void motor_esc__set_motor_speed(float motor_speed) {
  ...
  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, motor_speed*100);  //Set DC motor Duty cycle
}
